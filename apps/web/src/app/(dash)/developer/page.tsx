    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Build integrations with your AI content pipeline</p>
        </div>
      </div>
      ...
      {createdKey && (
        <div className="mb-6 bg-violet-50 border-2 border-violet-300 rounded-2xl p-5">
          <div className="flex items-start justify-between mb-2">
            <div>
              <p className="font-semibold text-violet-900">Key created! Copy it now — it won&apos;t be shown again.</p>
              <p className="text-xs text-violet-700 mt-0.5">Store this key securely. We cannot recover it.</p>
            </div>
            <button type="button" onClick={() => setCreatedKey(null)} className="text-violet-400 hover:text-violet-600 ml-4">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="font-mono text-sm bg-gray-900 text-green-400 p-3 rounded-lg break-all mb-3">
            {createdKey}
          </div>
          <button
            type="button"
      ...
      <div className="flex bg-gray-100 rounded-xl p-1 mb-6 w-fit">
        {(['keys', 'usage'] as const).map(t => (
          <button
            ...
            type="button"
            onClick={() => setTab(t)}
            className={`px-5 py-2 text-sm font-medium rounded-lg transition-colors capitalize ${tab === t ? 'bg-white shadow text-violet-700' : 'text-gray-500 hover:text-gray-700'}`}
          >
            {t === 'keys' ? <span className="flex items-center gap-1.5"><Key className="w-3.5 h-3.5" /> API Keys</span> : <span className="flex items-center gap-1.5"><BarChart2 className="w-3.5 h-3.5" /> Usage</span>}
          </button>
        ))}
      </div>
      ...
      {tab === 'keys' && (
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            ...
            >
              <span className="flex items-center gap-2"><Plus className="w-4 h-4 text-violet-600" /> Create New API Key</span>
              {showCreate ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
            </button>

            {showCreate && (
              <form onSubmit={(e) => { void handleCreate(e); }} className="border-t border-gray-100 px-5 py-4 space-y-4">
                <div>
          ...
          {keysLoading ? (
            ...
            <div className="space-y-3">
              {keys.map(key => (
                <div key={key.id} className="bg-white border border-gray-200 rounded-2xl p-5">
                      ...
                      {revokeConfirm === key.id ? (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 bg-red-500 text-white rounded-lg text-xs font-medium hover:bg-red-600 disabled:opacity-50 flex items-center gap-1"
                          >
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-600">
            Manage webhooks in{' '}
            <a href="/settings" className="text-violet-600 hover:underline font-medium">Settings → Developer Webhooks</a>.
          </div>
        </div>
      )}
          ...
          <div className="flex items-center gap-2">
            ...
            {([7, 14, 30, 90] as const).map(d => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${usageDays === d ? 'bg-violet-600 text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                {d}d
              </button>
            ))}
                      ...
                      <div key={k.keyId}>
                        ...
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${Math.max(2, (k.total / maxUsage) * 100)}%` }}
                          />
                        </div>
                      </div>
                    ))}
