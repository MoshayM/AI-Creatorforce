...
/** Accordion group listing library videos of one kind (long-form or Shorts). */
function KindVideosGroup({ channelId, kind, title, icon, renderVideo }: {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, error } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      api.library
        .listVideos(channelId, { cursor: pageParam as string | undefined, type: kind })
      ...
      {open && (
        <div className="mt-1.5 ml-4 space-y-1.5">
          ...
          {!isLoading && !error && videos.length === 0 && (
            <p className="text-center py-4 text-gray-500 text-sm">Nothing here yet — sync this channel from the Library page.</p>
          )}
...
/** Flat results across the whole library (videos + Shorts) while searching. */
function SearchResults({ channelId, q, renderVideo }: {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, error } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      api.library
        .listVideos(channelId, { cursor: pageParam as string | undefined, q })
        .then((r) => r.data as LibraryVideosPage),
    initialPageParam: undefined as string | undefined,
    getNextPageParam: (last) => last.nextCursor ?? undefined,
  });
  ...
  return (
    <div className="space-y-1.5">
      ...
      {!isLoading && !error && videos.length === 0 && (
        <p className="text-center py-4 text-gray-500 text-sm">No library videos match your search.</p>
      )}
      {videos.map(renderVideo)}
      {hasNextPage && <LoadMoreButton onClick={() => void fetchNextPage()} loading={isFetchingNextPage} />}
    </div>
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2">
          {/* Shorts and Videos browse separately; a search query flattens everything */}
          {q ? (
            <SearchResults channelId={channelId} q={q} renderVideo={renderVideo} />
          ) : (
            <>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 px-1">Shorts</p>
              <KindVideosGroup
                channelId={channelId}
                kind="short"
                title="All Shorts"
                icon={<Clapperboard className="w-4 h-4 text-brand-600 shrink-0" />}
                renderVideo={renderVideo}
              />
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 px-1 pt-2">Videos</p>
              {loadingPlaylists && (
                <div className="flex items-center gap-2 text-gray-500 py-4 justify-center text-sm">
