...
async function main() {
  ...
  try {
    ...
    console.log("Token obtained:", token ? "yes (" + token.substring(0, 30) + "...)" : "no");
  } catch (e) {
  ...
  console.log("Screenshot 1 saved: verify-proj-menu.png");

  const menuBtns = await page.locator('button[aria-label="Project options"]').all();
  console.log("Number of Project options buttons found:", menuBtns.length);

  if (menuBtns.length > 0) {
    const isVisible = await menuBtns[0].isVisible();
    const bbox = await menuBtns[0].boundingBox();
    console.log("First button visible:", isVisible, "bbox:", JSON.stringify(bbox));
    ...
    console.log("Screenshot 2 saved: verify-proj-dropdown.png");
  } else {
    console.log("No buttons found — checking page state");
