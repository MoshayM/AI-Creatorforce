...
- ✅ Channel-first import: Shorts Studio selects a channel (not a project); a manual "Import from library" modal (searchable, cursor-paginated, checkbox multi-select) imports only user-picked videos — metadata reused from `LibraryVideo` with live-YouTube fallback; imports land in an auto-created per-channel container project and dedupe across the channel's projects (`importFromChannel` + 6 unit tests, Wave 35)
